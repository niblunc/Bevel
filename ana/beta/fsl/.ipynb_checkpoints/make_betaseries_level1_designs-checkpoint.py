...
def make_file(sub_id, run, trial_id, output_dir):
    with open(os.path.join(deriv_dir,'design_files/beta1_v2_design.fsf'),'r') as infile:
        ...
        design_fileout = os.path.join(output_dir, "design_files/%s_run-%s_trial-%s_design1_choice.fsf"%(sub_id, run, trial_id))
...
def create_fsf():
    ...
    for sub_path in SUBJECTS:
        ...
        for run in RUNS: # SECOND LOOP -- looping through RUNS
            ...
            subj_trials = sorted(glob.glob(os.path.join(sub_path, "func", "onsets", "bevel*run0%s*choice*trial*.txt"%run)))
            if not subj_trials:
                pass
            else:
                ...
                for trial_file in subj_trials:
                    ...
                    #print(trial_id)
                    nuis_file = os.path.join(sub_path, "func", "onsets", "bevel%s_run0%s_choice_nuis%s.txt"%(_id, run, trial_id))
                    output_path = os.path.join(output_dir, "%s_run-%s_choice-%s"%(sub_id, run, trial_id))
