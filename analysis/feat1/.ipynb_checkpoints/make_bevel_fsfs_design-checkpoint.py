...
def make_file(key):
    for run in arglist["RUN"]:
        with open(os.path.join(deriv_dir,'design_files/design1.fsf'),'r') as infile:
            ...
            with open(os.path.join(outpath,'%s_task-%s_run-%s_no_reg.fsf'%(key,arglist['TASK'], run)),'w') as outfile:
                outfile.write(tempfsf)
            outfile.close()
        infile.close()
        
def fill_dict(sub, deriv_dir):
    print("IN METHOD")
    #task = arglist['TASK']
    ...
    runs = glob.glob(os.path.join(data_dir, "func" "*run*bold_brain.nii.gz"))
    for run in runs:
        print(run)
