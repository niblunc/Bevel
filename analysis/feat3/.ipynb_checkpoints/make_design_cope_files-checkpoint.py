...
def make_fsfs():
    ...
    for cope_num in range(1, num_of_copes+1):
        ...
        OUTPUTDIR = os.path.join(deriv_dir, 'group_ana/02_19/cope%s'%cope_num)
        ...
        with open(template_file, 'r') as infile:
            ...
            if arglist["SES"] == False:
                OUTFILE_PATH = os.path.join(deriv_dir, 'group_ana/02_19/cope%s.fsf'%cope_num)
            else:
                OUTFILE_PATH = os.path.join(deriv_dir, 'group_ana/02_19/cope%s_%s.fsf'%(cope_num, arglist["SES"]))
            #print(tempfsf)
