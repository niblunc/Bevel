...
def combine_timeseries():
    roi_dict = {}
    outpath = "/projects/niblab/bids_projects/Experiments/Bevel/derivatives/betaseries/output/roi_v2/timeseries/by_sub"
    roi_dir = glob.glob(os.path.join(
        "/projects/niblab/bids_projects/Experiments/Bevel/derivatives/betaseries/output/roi_v2/timeseries/by_roi/*.txt"))
...
def pull_timeseries():
    # get nifti files
    out_dir = "/projects/niblab/bids_projects/Experiments/Bevel/derivatives/betaseries/output/roi_v2/timeseries/by_roi"
    file_list = glob.glob(
        os.path.join("/projects/niblab/bids_projects/Experiments/Bevel/derivatives/betaseries/output/niftis",
                     'sub-0*.nii.gz'))
    print("We have {} nifti files.".format(len(file_list)))
    beta_text_file = "/projects/niblab/bids_projects/Experiments/Bevel/derivatives/betaseries/betaseries_rois_ofc_fixed.txt"
...
def concat_trials(files, trial_dict, output_dir):
    ...
    for sub in trial_dict:
        ...
        print(sub)
        pe1 = glob.glob(os.path.join(
            '/projects/niblab/bids_projects/Experiments/Bevel/derivatives/{}/func/Analysis/feat1/betaseries/run-*/{}_run-*_trial-*.feat/stats/pe1.nii.gz'.format(
                sub, sub)))
        for run in trial_dict[sub]:
            ...
            for trial in trial_dict[sub][run]:
                if trial == "reward":
                    temp = [x for x in pe1 if
                            int(x.split("/")[-3].split("_")[2].split("-")[1].split('.')[0]) in trial_dict[sub][run][
                                trial]]
                    for x in temp:
...
def main():
    ...
    subs = glob.glob("/projects/niblab/bids_projects/Experiments/Bevel/derivatives/sub-0*")
    good_subs = [x.split("/")[-1] for x in subs]
    ...
    nifti_run_files = glob.glob(os.path.join(output_dir, "*.nii.gz"))
    # print(nifti_run_files)
    pull_timeseries()
    combine_timeseries()
